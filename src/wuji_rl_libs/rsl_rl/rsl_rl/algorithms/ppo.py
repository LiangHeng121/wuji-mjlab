...
class PPO:
    ...
    def update(self) -> dict[str, float]:
        ...
        # Clamp the standard deviation of the actor if a minimum is set
        if self.min_std is not None:
            log_min = math.log(self.min_std)
            for p in self.actor.parameters():
                if p.shape == self.actor.output_distribution_params[1].shape:
                    p.data.clamp_(min=log_min)
