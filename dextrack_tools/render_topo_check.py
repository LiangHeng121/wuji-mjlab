"""Spot-check 一起-TOPO apple: render borderline/clear/fail cases with NEW verdict
labels for human validation. 3x2 grid. MUJOCO_GL=egl."""
...
G, Y, R = (140, 255, 140), (255, 220, 120), (255, 140, 140)
CELLS = [
  ("ori_grab_s6_apple_pass_1", "pass  PASS 0.51 (barely)", Y),
  ("ori_grab_s7_apple_lift", "lift  PASS 0.51 (was-fail)", Y),
  ("ori_grab_s9_apple_lift", "lift  PASS 0.70 (was-fail)", Y),
  ("ori_grab_s2_apple_lift", "lift  PASS 0.83", G),
  ("ori_grab_s4_apple_pass_1", "pass  PASS 1.00", G),
  ("ori_grab_s2_apple_eat_1", "eat   FAIL 0.48", R),
]
W, H = 512, 384
...
def lab(img, txt, color):
  ...
  d.rectangle([0, 0, 340, 30], fill=(0, 0, 0)); d.text((6, 3), txt, fill=color, font=f)
...
gif = f"{OUTDIR}/topo_check_grid.gif"
imageio.mimwrite(gif, out, fps=20, loop=0)
imageio.mimwrite(f"{OUTDIR}/topo_check_grid.mp4", out, fps=20, quality=9)
